...
class _UpdateTaskPageState extends State<UpdateTaskPage> {
  ...
  Future<void> _loadCategories() async {
    ...
    setState(() {
      _categories =
          snapshot.docs
              .map((d) => {'id': d.id, 'name': d['name'], ...d.data()})
              .toList();
    });
  }
    ...
    if (!doc.exists) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.taskNotFound)),
        );
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            AppLocalizations.of(context)!.errorUpdating + e.toString(),
          ),
        ),
      );
    } finally {
  ...
  Future<void> _delete() async {
    final confirm = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              AppLocalizations.of(context)!.areYouSureYouWantToRemoveThisTask,
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.taskRemoved)),
      );
      Navigator.pop(context);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            AppLocalizations.of(context)!.errorRemovingTask + e.toString(),
          ),
        ),
      );
    }
